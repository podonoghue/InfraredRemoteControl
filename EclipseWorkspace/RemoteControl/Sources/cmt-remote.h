...
public:
   ...
   struct Parameters {
      ...
      Ticks    zeroHigh;      //< High time for a zero
      Ticks    zeroLow;       //< Low time for a zero
      Ticks    oneHigh;       //< High time for a one
      Ticks    oneLow;        //< Low time for a one
      Ticks    startHigh;     //< High time for start
      Ticks    startLow;      //< Low time for start
      Ticks    repeatTime;    //< Time to repeated Tx
      Ticks    repeatHigh;    //< High time for a repeat bit
      Ticks    repeatLow;     //< Low time for a repeat bit
...
protected:
   ...
   static void start(unsigned delay=0) {
      ...
      if (delay>0) {
         console.writeln("Delaying ", delay, "ms");
//         waitMS(delayTime);
      }
      waitUntilComplete();
   }
   ...
   static inline volatile bool busyFlag = false;
...
private:
   ...
   static constexpr Parameters myParameters = {
         ...
         /* zeroHigh       1   */  1*564_ticks,
         /* zeroLow       -1   */  1*564_ticks,
         /* oneHigh        1   */  1*564_ticks,
         /* oneLow        -3   */  3*564_ticks,
         /* startHigh     16   */ 16*564_ticks,
         /* startLow      -8   */  8*564_ticks,
         /* repeatTime   108m  */ 108000_ticks,
         /* repeatHigh    16   */ 16*564_ticks,
         /* repeatLow     -4   */  4*564_ticks,
 ...
 *  Packet formats:
 *
 *    20 bit   <--  16 bits  -->      <--  8 bits  --> <--  8 bits  --> <- 4 bits ->
 *    +-------+-----------------+---------------------+----------------+------------+----+
 *    | Start |    Device ?     | MS |     F          |      ~F        |    =7      |Stop|
 *  --+-------+-----------------+---------------------+----------------+------------+----+--
 */
...
private:
   ...
   /// Constant describing transmission
   /// IRP notation: {38k,500u}<1,-1|1,-3>(9,-9,D:8,S:8,1,-9,E:4,F:8,~F:8,1,-118)+
   static constexpr Parameters myParameters = {
         /* carrier       */    38_kHz,

         // Following are in CMT ticks = 1us, multiples of 500u
         /* zeroHigh      1  */  1*500_ticks,
         /* zeroLow      -1  */  1*500_ticks,
         /* oneHigh       1  */  1*500_ticks,
         /* oneLow       -3  */  3*500_ticks,
         /* startHigh     9  */  9*500_ticks,
         /* startLow     -9  */  9*500_ticks,
         /* repeatTime       */ 120000_ticks,
         /* repeatHigh    9  */  9*500_ticks, // Full repeat
         /* repeatLow    -9  */  9*500_ticks,
...
private:
   ...
   static constexpr Parameters myParameters = {
         ...
         /* zeroHigh      */  1*600_ticks,
         /* zeroLow       */  1*600_ticks,
         /* oneHigh       */  2*600_ticks,
         /* oneLow        */  1*600_ticks,
         /* startHigh     */  4*600_ticks,
         /* startLow      */  1*600_ticks,
...
//      SOURCE_6             = makeSonyCode(12,    0x49, 0x01), // maybe?
   ...
   static void send(Code cmtCommand, unsigned delay, unsigned repeats=3) {
      ...
      console.writeln("IrRemote: Sony-TV: 0x", (uint32_t)cmtCommand, Radix_16);
